...
`csprobe read` (ZenStates-Core / WinRing0). Requires an elevated process —
...
from config import CSPROBE_EXE, TCTL_SMN_ADDR, TEMP_SAMPLE_INTERVAL

logger = logging.getLogger(__name__)

# csprobe success line: "SMN 0x059800 = 0x693B0000 (1765474304)"
...
def read_tctl() -> float:
    ...
    if not CSPROBE_EXE.exists():
        raise TemperatureError(f"csprobe.exe not found at {CSPROBE_EXE}")
    try:
        result = subprocess.run(
            [str(CSPROBE_EXE), "read", hex(TCTL_SMN_ADDR)],
            capture_output=True, text=True, encoding='utf-8', errors='replace',
            timeout=30, creationflags=getattr(subprocess, "CREATE_NO_WINDOW", 0)
        )
    except subprocess.TimeoutExpired:
        raise TemperatureError(f"csprobe read timed out after 30s")

    out = (result.stdout or "") + (result.stderr or "")
    if "INIT-FAIL" in out:
        raise TemperatureError(
            "csprobe cannot start the WinRing0 kernel driver "
            "(run from an elevated process)")
    if result.returncode != 0:
        raise TemperatureError(
            f"csprobe read failed (exit {result.returncode}): {out.strip()[:200]}")

    m = _SMN_RE.search(out)
    if not m:
        raise TemperatureError(f"cannot parse csprobe output: {out.strip()[:200]}")
